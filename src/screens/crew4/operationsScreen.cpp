...
: GuiOverlay(owner, "", colorConfig.background)
{
        ...
        ->setCallbacks(
            [this, science](sf::Vector2f position) { // Down
              ...
              if (mode == TargetSelection &&
                  science->targets.getWaypointIndex() > -1 && my_spaceship) {
    ...
    // Manage waypoints.
    place_waypoint_button =
        new GuiButton(science->radar_view, "WAYPOINT_PLACE_BUTTON",
                      "Place waypoint", [this]() { mode = WaypointPlacement; });
    place_waypoint_button->setPosition(-270, -70, ABottomRight)->setSize(200, 50);

    delete_waypoint_button = new GuiButton(
        science->radar_view, "WAYPOINT_DELETE_BUTTON", "Delete waypoint",
        [science]() {
